      ...
      <div className="h-full flex flex-col gap-4 animate-in fade-in">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] shadow-xl flex items-center justify-between border-b-8 border-slate-100">
           <div className="flex items-center gap-6">
              <div className="bg-slate-900 text-white p-5 rounded-[1.5rem] text-center min-w-[140px]">
              ...
              <div>
                 <h3 className="text-xl font-black uppercase italic text-slate-800">Bảng điều phối</h3>
                 <p className="text-xs font-bold text-slate-400 uppercase italic">Học sinh online: {connectedCount}</p>
              </div>
           </div>
              ...
              <div className="flex-1 min-h-0">
                {currentProb ? (
                  ...
                  <div className="h-full bg-white rounded-[2.5rem] flex items-center justify-center border-4 border-dashed border-slate-200">
                    <p className="font-black uppercase italic text-slate-300">Vui lòng chọn bộ đề trước</p>
                  </div>
                )}
              </div>
              <div className="bg-emerald-50 p-6 rounded-[2rem] border-2 border-emerald-100">
                 <div className="text-[10px] font-black text-emerald-600 uppercase mb-2">Đáp án đúng & Giải thích</div>
                 <div className="text-sm font-bold text-slate-700 italic">
                   <LatexRenderer content={`**Đáp án:** ${currentProb?.correctAnswer} \n\n ${currentProb?.explanation}`} />
                 </div>
              </div>
           </div>
           ...
           <div className="lg:col-span-5 flex flex-col h-full bg-slate-900 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col h-full">
                 ...
                 <textarea 
                    ...
                    placeholder="Gõ nội dung giảng bài tại đây... (Hỗ trợ $ $ cho công thức)"
                    value={liveNote}
                    onChange={(e) => handleNoteChange(e.target.value)}
                 />
                 <div className="h-40 bg-white/5 rounded-2xl p-4 overflow-y-auto border border-white/5">
                    <div className="text-[9px] font-black text-slate-500 uppercase mb-2">Xem trước hiển thị:</div>
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-4">
              <div className="md:col-span-10">
                ...
                <input type="text" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-3 font-black" value={currentTitle} onChange={e => setCurrentTitle(e.target.value)} />
              </div>
              <div className="md:col-span-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Khối</label>
                <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-3 font-black" value={currentGrade} onChange={e => setCurrentGrade(e.target.value)}>
                  {['10', '11', '12'].map(g => <option key={g} value={g}>K{g}</option>)}
                </select>
              </div>
            </div>
          </div>
          <button onClick={() => onSaveSet(currentTitle, !loadedSetId, 'Vật lý', currentGrade)} className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-black uppercase italic shadow-lg">Lưu đề</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        <div className="lg:col-span-3 flex flex-col gap-4 overflow-hidden">
          <div className="bg-white p-4 rounded-3xl border-4 border-slate-50 shadow-sm flex items-center gap-3 overflow-x-auto no-scrollbar">
             {rounds?.map((r, i) => (
                <button key={i} onClick={() => { setActiveRoundIdx(i); setEditingIdx(null); }} className={`px-6 py-3 rounded-2xl text-[11px] font-black uppercase border-2 shrink-0 ${activeRoundIdx === i ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-400'}`}>Vòng {r.number}</button>
             ))}
          ...
          <div className="bg-white rounded-[2rem] p-5 shadow-md border-2 border-slate-50 flex-1 overflow-y-auto no-scrollbar space-y-2">
              {rounds?.[activeRoundIdx]?.problems?.map((p, i) => (
                <button key={p.id} onClick={() => setEditingIdx(i)} className={`w-full p-4 rounded-2xl text-left border-4 transition-all ${editingIdx === i ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-50 border-white text-slate-500'}`}>
              ...
              }} className="w-full py-4 border-2 border-dashed border-slate-200 text-slate-400 rounded-2xl font-black uppercase text-[10px]">+ THÊM CÂU</button>
          </div>
        </div>
        ...
        <div className="lg:col-span-9 bg-white rounded-[3rem] shadow-xl p-10 overflow-y-auto no-scrollbar border-4 border-slate-50 relative">
            ...
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in">
                <div className="flex justify-between items-center border-b-4 border-slate-50 pb-6">
                  ...
                  <button onClick={() => {
                    ...
                    setRounds(updated);
                    setEditingIdx(null);
                  }} className="text-red-500 font-black uppercase text-[10px]">Xóa câu ✕</button>
                </div>
          ...
          ) : <div className="h-full flex flex-col items-center justify-center opacity-20 text-center"><div className="text-[10rem] mb-6 select-none">✏️</div><p className="font-black uppercase italic text-2xl text-slate-400">CHỌN CÂU HỎI ĐỂ SOẠN THẢO</p></div>}
        </div>
      </div>
    </div>
  );
