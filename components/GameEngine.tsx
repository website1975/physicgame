...
const QUESTION_REVEAL_TIME = 4; // Reading phase
  ...
  const startProblem = useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (count <= 0) {
        ...
        // Start by just revealing the question
        setGameState('STARTING_ROUND_REVEAL');
        setTimeout(() => {
            if (isArenaA || isTeacherRoom) {
              setGameState('ANSWERING');
            } else {
              setGameState('WAITING_FOR_BUZZER');
            }
        }, QUESTION_REVEAL_TIME * 1000);
      }
    }, 1000);
    ...
    if (gameState === 'FEEDBACK' && feedbackTimer === 0) {
      if (isArenaA) { handleNext(); } 
        ...
        if (isMaster) {
          ...
          setTimeout(() => { if (gameStateRef.current === 'FEEDBACK') handleNext(); }, 3000); 
        }
      }
    }
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0 overflow-hidden">
        <div className="lg:col-span-7 h-full overflow-hidden">
          {gameState === 'ROUND_INTRO' ? (
            ...
            <ProblemCard problem={currentProblem} isPaused={isWhiteboardActive} isHelpUsed={isHelpUsed} hideOptions={gameState === 'STARTING_ROUND_REVEAL' || gameState === 'WAITING_FOR_BUZZER'} />
          )}
        </div>
        <div className="lg:col-span-5 bg-white rounded-[3rem] p-8 shadow-xl flex flex-col relative h-full overflow-hidden">
          {gameState === 'ROUND_INTRO' ? (
          ...
          ) : gameState === 'STARTING_ROUND_REVEAL' ? (
             <div className="h-full flex flex-col items-center justify-center text-center">
                <div className="text-6xl mb-6 animate-pulse">📖</div>
                <h3 className="text-2xl font-black text-slate-800 uppercase italic">HÃY ĐỌC KỸ CÂU HỎI...</h3>
                <p className="text-slate-400 font-bold italic text-xs mt-2 uppercase">Chuẩn bị nhấn chuông!</p>
             </div>
          ) : gameState === 'FEEDBACK' ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in">
               ...
               <h3 className="text-3xl font-black text-slate-800 uppercase italic mb-8">NHẤN CHUÔNG NHANH!</h3>
               <button onClick={() => !buzzerWinner && (channelRef.current?.send({ type: 'broadcast', event: 'buzzer_signal', payload: { player: playerName, playerId: myUniqueId } }), setBuzzerWinner('YOU'), setGameState('ANSWERING'), setTimeLeft(20))} className="w-56 h-56 bg-red-600 rounded-full border-[12px] border-red-800 shadow-[0_15px_0_#991b1b,0_25px_50px_rgba(220,38,38,0.3)] hover:scale-105 active:translate-y-3 transition-all flex items-center justify-center group relative">
                 <div className="absolute inset-0 rounded-full bg-white opacity-0 group-hover:opacity-10 transition-opacity"></div>
                 <span className="text-white font-black text-4xl uppercase italic text-center drop-shadow-lg">BẤM!</span>
               </button>
               <div className="mt-8 text-[10px] font-black text-slate-300 uppercase tracking-widest animate-pulse">Ai nhấn trước sẽ được trả lời</div>
            </div>
          ) : gameState === 'ANSWERING' && buzzerWinner === 'OPPONENT' ? (
            <div className="h-full flex flex-col items-center justify-center text-center animate-in slide-in-from-right"><div className="w-24 h-24 border-[10px] border-slate-100 border-t-blue-600 rounded-full animate-spin mb-8"></div><div className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl w-full border-b-[10px] border-slate-950"><h3 className="text-2xl font-black uppercase italic mb-2 text-blue-400">TẠM DỪNG!</h3><p className="font-bold text-slate-400 italic text-sm">Đối thủ đang nhập đáp án...</p></div></div>
          ) : (
            <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in">
                ...
                <button onClick={() => submitAnswer()} className="w-full py-6 bg-blue-600 text-white rounded-[1.8rem] font-black italic text-xl mt-4 shadow-xl border-b-8 border-blue-800 hover:scale-105 transition-all">NỘP ĐÁP ÁN ✅</button>
            </div>
          )}
        </div>
      </div>
